...
my @files=glob("/home/takahagi/ALCA_from_20190726/Bd21_col_sta_Bd14-1/DESeq2_from_20190813/diploid_vs_tetrapolyploid/*.txt");
foreach my $each_file(@files){
	#print"$each_file\n";
	
	if($each_file=~/Bd_col\.\d+\.txt$|Bd_Bh\.\d+\.txt$|Bs_Bh\.\d+\.txt$/){
		#print"$each_file\n";
		
		$each_file=~/DESeq2_from_20190813\/diploid_vs_tetrapolyploid\/(.+)_(.+)\.(\d+)\.txt$/;
		my $sp1=$1;
		my $sp2=$2;
		my $time=$3;
		print"$sp1\t$sp2\t$time\n";
		...
		my $out_up="diploid_vs_tetrapolyploid/$sp1"."_$sp2.$time.DEG_list.up_$sp2.txt";
		my $out_down="diploid_vs_tetrapolyploid/$sp1"."_$sp2.$time.DEG_list.down_$sp2.txt";
