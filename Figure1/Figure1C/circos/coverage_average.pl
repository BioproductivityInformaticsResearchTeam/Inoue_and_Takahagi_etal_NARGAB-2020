...
sub get_genome{
	...
	foreach my $each(@_){	#@_=@Bd
		...
		#for(my $rep=0;$rep<=$each;$rep+=5000000){	#5mb
		#for(my $rep=0;$rep<=$each;$rep+=10000000){	#10mb
			#my $end=$rep+99999;
			my $end=$rep+999999;
			#my $end=$rep+4999999;
			#my $end=$rep+9999999;
...
open(A,"../4xBd21_genome.sort.rmdup.f2.coverage.txt");
<A>;
while(my $line=<A>){
	$line=~s/\r//;
	chomp $line;
